...
class PSO(object):
    def __init__(self, num_city, data, dis_mat=None, iterations=500, population_size=200, w=0.8, c1=2, c2=2):
        self.iter_max = iterations  # 迭代数目
        self.num = population_size  # 粒子数目
    ...
    def pso(self):
        ...
        for cnt in range(1, self.iter_max):
            ...
            # print(times[cnt-1], self.best_l)
            self.iter_x.append(cnt)
            self.iter_y.append(self.best_l)
        return self.best_l, self.best_path

    def write_tsp_format(self, path, location, best_path):
        # 获取脚本所在目录
        ...
        with open(full_path, 'w') as file:
            file.write("NAME : result_PSO.tour\n")
            file.write(f"COMMENT : Length = {len(best_path)}\n")
            file.write("COMMENT : Found by PSO\n")
            file.write("TYPE : TOUR\n")
            file.write(f"DIMENSION : {len(location)}\n")
    ...
    def run(self):
        ...
        # 保存路线到文件
        output_filename = f"result_single_PSO_{self.num_city}.tsp"
        self.write_tsp_format(output_filename, self.location, best_path)
        print(f"PSO tour saved to {output_filename}")
        # 画出最终路径
